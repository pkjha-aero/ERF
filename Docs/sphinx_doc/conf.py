...
# built documents.
#
# Derived from git by the same helper the build uses to stamp the binary
# (Tools/gen_erf_version.py), so the documentation cannot claim a different
# version from the code it describes. See VERSION_MANAGEMENT.md.
_erf_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
sys.path.insert(0, os.path.join(_erf_root, 'Tools'))
try:
    from gen_erf_version import version_string
    # Full describe, e.g. "26.06-472-g0dcca5c72", or "unknown" without git.
    release = version_string(_erf_root)
except ImportError:
    release = u'unknown'
# The short X.Y version: the release tag the build descends from.
